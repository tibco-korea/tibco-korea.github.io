...
```
> `image: chungsju/bwce-mon:latest` 는 public docker hub에 push한 Image repository 이름입니다.
> `value: postgres://bwuser:bwuser@postgres-bwce-mon-svc:5432/postgres` 에서 앞서 Monitoring 결과 DB에서 설정한 ID/PW 및 Service resource의 이름(postgres-bwce-mon-svc) 및 port를 입력합니다. 
...
```
> `image: chungsju/provider:1.0` 는 public docker hub에 push한 Image repository 이름입니다.
> `BW_APP_MONITORING_CONFIG` 의 값으로 앞서 Monitoring Service resource의 이름(bwce-mon-svc) 및 port를 입력합니다. 
        ...
        - name: consumer
          ...
          env:
           ...
           - name: BW_APP_MONITORING_CONFIG
             value: '{"url":"http://bwce-mon-svc:8080"}'
```
> `image: chungsju/consumer:1.0` 는 public docker hub에 push한 Image repository 이름입니다.
> `BW_APP_MONITORING_CONFIG` 의 값으로 앞서 Monitoring Service resource의 이름(bwce-mon-svc) 및 port를 입력합니다. 
> `PROVIDER_HOST` 의 값으로 앞서 Provider Service resource의 이름(provider-svc)을  입력합니다. 
